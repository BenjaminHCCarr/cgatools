...
template <bool Selector, typename T1, typename T2> struct TypeSelector;
template <typename T1, typename T2> struct TypeSelector<true,  T1, T2> { typedef T1 Type; };
template <typename T1, typename T2> struct TypeSelector<false, T1, T2> { typedef T2 Type; };
...
namespace variants {
    ...
    class AlternativeCallsAnnotationRetriever : public AnnotationRetriever
    {
    public:
        AlternativeCallsAnnotationRetriever(VariantFileVcfRecordWriter* conv)
            : conv_(conv)
        {
        }
        ...
        std::string getAnnotation(const Call& call, const Locus& locus)
        {
            if (0 == call.alternativeCalls_.size())
                return "";

            const VariantFileVcfRecordWriter::VcfData& rec = conv_->getRecord();
            const CrrFile& crr = conv_->getCrr();
            string prefix, suffix;
            if (rec.range_.beginLocation() < call.range_.beginLocation())
                prefix = crr.getSequence(Range(rec.range_.beginLocation(), call.range_.beginLocation()));
            if (call.range_.endLocation() < rec.range_.endLocation())
                suffix = crr.getSequence(Range(call.range_.endLocation(), rec.range_.endLocation()));

            string result;
            for(size_t ii=0; ii<call.alternativeCalls_.size(); ii++)
            {
                if (ii > 0)
                    result += "&";
                result += prefix;
                result += call.alternativeCalls_[ii].alleleSeq_;
                result += suffix;
                result += "|";
                result += boost::lexical_cast<string>(call.alternativeCalls_[ii].score_);
            }
            return result;
        }
    ...
    class AlleleFreqAnnotationRetriever : public AnnotationRetriever
    {
    public:
        std::string getAnnotation(const Call& call, const Locus& locus)
        {
            return call.alleleFreq_;
        }
    };
    ...
    template <bool SortAndUniquify >
    class JoinAnnotationAccumulator : public AnnotationAccumulator
    {
    public:
        JoinAnnotationAccumulator(const char* outSep, const char* emptyRepl = NULL)
            : outSep_(outSep), emptyRepl_(emptyRepl)
        {
        }
    ...
    private:
        typename TypeSelector<SortAndUniquify, std::set<std::string>, std::vector<std::string> >::Type ann_;
        const char* outSep_;
        const char* emptyRepl_;
    };
    ...
    class DotAnnotationAccumulator : public AnnotationAccumulator
    {
    public:
        DotAnnotationAccumulator()
        {
            init();
        }
                      ...
                      const std::vector< std::vector<SubFieldCall> >& calls)
        {
            ...
            for(size_t ii=0; ii<calls.size(); ii++)
            {
                for(size_t jj=0; jj<calls[ii].size(); jj++)
                {
                    ...
                    {
                        if (std::find(filters.begin(), filters.end(), varFilter) == filters.end())
                            filters.push_back(varFilter);
                    }
                }
            }
            ...
            for(size_t ii=0; ii<calls.size(); ii++)
            {
                for(size_t jj=0; jj<calls[ii].size(); jj++)
                {
                    ...
                    {
                        if (std::find(filters.begin(), filters.end(), "SQLOW") == filters.end())
                            filters.push_back("SQLOW");
                    }
            ...
            if (filters.size() > 1)
                filters.erase(std::remove(filters.begin(), filters.end(), "PASS"), filters.end());
            if (0 == filters.size())
                return ".";
            else
                return boost::join(filters, ";");
        }
    };
          ...
          locusCount_(0)
    {
            ...
            if (hasLocusSpecificXRef)
            {
                addAnnotation(
                    ...
                    new GlobalSubField(
                        ...
                        PAA(new JoinAnnotationAccumulator<true>(",", "")),
                        vector<bool>(),
        ...
        if (std::find(fn.begin(), fn.end(), "AF") != fn.end())
        {
            addAnnotation(
                infoAnn_,
                new AlleleSubField(
                    PAR(new AlleleFreqAnnotationRetriever()),
                    PAA(new JoinAnnotationAccumulator<false>("&", "?")),
                    vector<bool>(),
                    VcfSubFieldHeaderRecord::VCF_INFO,
                    "AF",
                    "A",
                    "String",
                    "Allele frequency, or &-separated frequencies for complex variants "
                    "(in latter, '?' designates unknown parts)"));
        }
        if (std::find(fn.begin(), fn.end(), "CGA_ALTCALLS") != fn.end())
        {
            addAnnotation(
                gtAnn_,
                new AlleleSubField(
                    PAR(new AlternativeCallsAnnotationRetriever(this)),
                    PAA(new DotAnnotationAccumulator()),
                    vector<bool>(),
                    VcfSubFieldHeaderRecord::VCF_FORMAT,
                    "CGA_ALTCALLS",
                    "2",
                    "String",
                    "Alternative call sequences and scores"));
        }
        ...
        if (std::find(fn.begin(), fn.end(), "CGA_CEGL") != fn.end())
        {
            ...
            addAnnotation(
                ...
                new GenotypeLikelihoodSubField(
                    "CGA_CEGL",
                    "Calibrated Genotype Likelihood, Equal Allele Fraction Assumption",
                    this,
                    rec_.cehq_,
                    &hasCalibration_));
        }
        ...
        if (std::find(fn.begin(), fn.end(), "CGA_SOMF") != fn.end() &&
            ach.find("fisherSomatic") != ach.end())
        {
            addAnnotation(
                gtAnn_,
                new GlobalSubField(
                    PAR(new LocusAnnotationRetriever("fisherSomatic")),
                    PAA(new FirstNonEmptyAccumulator()),
                    vector<bool>(),
                    VcfSubFieldHeaderRecord::VCF_FORMAT,
                    "CGA_SOMF",
...
//                                  << endl;
    ...
    void VariantFileVcfRecordWriter::appendScores(const PhasedAllele& allele, size_t gIdx)
    {
        ...
        rec_.cehq_[gIdx].push_back(static_cast<int32_t>(cehq));
    }
    ...
    VariantFileVcfRecordSource::getSubFieldHeaderRecords() const
    {
        ...
        result.push_back(VcfSubFieldHeaderRecord(
                             ...
                             "Call is homozygous and the varScoreVAF is less than 20dB, or "
                             "the call is not homozygous and the varScoreVAF is less than 40dB"));
        result.push_back(VcfSubFieldHeaderRecord(
                             VcfSubFieldHeaderRecord::VCF_FILTER, "SQLOW", "", "",
                             "Somatic variant has somaticScore < -10"));
        result.push_back(VcfSubFieldHeaderRecord(
                             VcfSubFieldHeaderRecord::VCF_FILTER, "FET30", "", "",
                             "Fisher somatic score < 30"));
        result.push_back(VcfSubFieldHeaderRecord(
                             VcfSubFieldHeaderRecord::VCF_FILTER, "AMBIGUOUS", "", "",
                             "Read evidence does not strongly distinguish"
                             " multiple non-reference candidate alleles"));
        return result;
    }
