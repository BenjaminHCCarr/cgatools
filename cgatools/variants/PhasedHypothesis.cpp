...
namespace cgatools { namespace variants {
                                             ...
                                             std::vector<AlleleDiffSegment>& segs)
    {
        ...
        // Merge 0-length segments with adjacent non-ref segments.
        for(size_t ii=0; ii<segs.size(); ii++)
        {
            if (segs[ii].ref_.first == segs[ii].ref_.second)
            {
                bool mergeLeft  = ii   > 0           && isSticky(lhs, rhs, segs[ii-1]);
                bool mergeRight = ii+1 < segs.size() && isSticky(lhs, rhs, segs[ii+1]);
                if (mergeLeft)
                {
                    segs[ii-1].lhs_.second = segs[ii].lhs_.second;
                    segs[ii-1].rhs_.second = segs[ii].rhs_.second;
                    segs[ii-1].ref_.second = segs[ii].ref_.second;
                    segs[ii-1].lhsCalls_.second = segs[ii].lhsCalls_.second;
                    segs[ii-1].rhsCalls_.second = segs[ii].rhsCalls_.second;
                    segs.erase(segs.begin()+ii);
                    ii--;
                }
                if (mergeRight)
                {
                    segs[ii].lhs_.second = segs[ii+1].lhs_.second;
                    segs[ii].rhs_.second = segs[ii+1].rhs_.second;
                    segs[ii].ref_.second = segs[ii+1].ref_.second;
                    segs[ii].lhsCalls_.second = segs[ii+1].lhsCalls_.second;
                    segs[ii].rhsCalls_.second = segs[ii+1].rhsCalls_.second;
                    segs.erase(segs.begin()+ii+1);
                }
            }
        }
    }

    bool PhasedHypothesis::isSticky(const PhasedAllele& lhs,
                                    const PhasedAllele& rhs,
                                    const AlleleDiffSegment& seg)
    {
        if (seg.lhsCalls_.second - seg.lhsCalls_.first > 1)
            return true;
        if (seg.rhsCalls_.second - seg.rhsCalls_.first > 1)
            return true;
        if (seg.lhsCalls_.second - seg.lhsCalls_.first == 1)
        {
            const Call& call = *(lhs.calls()[seg.lhsCalls_.first]);
            if ( ! ("=" == call.alleleSeq_ || "?" == call.alleleSeq_ || call.alleleSeq_ == call.reference_) )
                return true;
        }
        if (seg.rhsCalls_.second - seg.rhsCalls_.first == 1)
        {
            const Call& call = *(rhs.calls()[seg.rhsCalls_.first]);
            if ( ! ("=" == call.alleleSeq_ || "?" == call.alleleSeq_ || call.alleleSeq_ == call.reference_) )
                return true;
        }
        return false;
