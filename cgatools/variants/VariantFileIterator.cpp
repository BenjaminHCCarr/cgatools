...
namespace cgatools { namespace variants {
    ...
    {
    public:
        VarQualityCallFilter(const std::string& excludeQuality)
            : excludeQuality_(excludeQuality)
        {
            if ("VQHIGH" == excludeQuality_)
                excludeQuality_ = "PASS";
        }

        bool passesFilter(const Locus& locus, const Call& call) const
        {
            return 1 != call.varFilter_.size() || call.varFilter_[0] != excludeQuality_;
        }

    private:
        std::string excludeQuality_;
    };

    class VarFilterNeCallFilter : public VariantFileIterator::CallFilter
    {
    public:
        VarFilterNeCallFilter(const std::string& excludeFilter)
            : excludeFilter_(excludeFilter)
        {
        }

        bool passesFilter(const Locus& locus, const Call& call) const
        {
            return 1 != call.varFilter_.size() || call.varFilter_[0] != excludeFilter_;
        }

    private:
        std::string excludeFilter_;
    };

    class VarFilterContainsCallFilter : public VariantFileIterator::CallFilter
    {
    public:
        VarFilterContainsCallFilter(const std::string& excludeFilters)
        {
            boost::split(excludeFilters_, excludeFilters, boost::is_any_of("|"));
        }

        bool passesFilter(const Locus& locus, const Call& call) const
        {
            BOOST_FOREACH(const string& filt, excludeFilters_)
            {
                if (std::find(call.varFilter_.begin(), call.varFilter_.end(), filt) != call.varFilter_.end())
                    return true;
            }
            return false;
        }
    ...
    private:
        std::vector<std::string> excludeFilters_;
    };
    ...
    VariantFileIterator::parseFilter(const std::string& andPart) const
    {
        ...
        if ("hom" == andPart)
            ...
            result.reset(new VarQualityCallFilter(andPart.substr(12)));
        else if (boost::starts_with(andPart, "varFilter!="))
            result.reset(new VarFilterNeCallFilter(andPart.substr(11)));
        else if (boost::starts_with(andPart, "varFilter contains "))
            result.reset(new VarFilterContainsCallFilter(andPart.substr(19)));
        if (0 == result.get())
    ...
    void VariantFileIterator::applyNoCall(Call& call)
    {
        ...
        call.varFilter_.clear();
        call.alternativeCalls_.clear();
        call.hapLink_ = "";
        call.xRef_ = "";
    }
    ...
    void VariantFileIterator::fillOlplLocus()
    {
        ...
        else
        {
            for (uint16_t hap = 0; hap < d.ploidy_; ++hap)
            {
                ...
                c.varFilter_   = d.alleleVarFilter_  [hap];
                c.hapLink_ = d.hapLink_[hap];
                c.xRef_    = d.xRef_[hap];
                c.alleleFreq_ = d.alleleFreq_[hap];
                c.alternativeCalls_ = d.alleleAlternativeCalls_[hap];
    ...
    void VariantFileIterator::addOlplParsers()
    {
        ...
        if ( df.hasField("allele1VarQuality") && !df.hasField("allele1VarFilter") )
        {
            df.addField(Call::VarQualityField("allele1VarQuality", &d.alleleVarFilter_[0]),
                        DelimitedFile::FPT_OPTIONAL);
        }
        else
        {
            df.addField(Call::VarFilterField("allele1VarFilter", &d.alleleVarFilter_[0]),
                        DelimitedFile::FPT_OPTIONAL);
        }
        if ( df.hasField("allele2VarQuality") && !df.hasField("allele2VarFilter") )
        {
            df.addField(Call::VarQualityField("allele2VarQuality", &d.alleleVarFilter_[1]),
                        DelimitedFile::FPT_OPTIONAL);
        }
        else
        {
            df.addField(Call::VarFilterField("allele2VarFilter", &d.alleleVarFilter_[1]),
                        DelimitedFile::FPT_OPTIONAL);
        }
        df.addField(StringField("allele1Freq", &d.alleleFreq_[0]),
                    DelimitedFile::FPT_OPTIONAL);
        df.addField(StringField("allele2Freq", &d.alleleFreq_[1]),
                    DelimitedFile::FPT_OPTIONAL);
        df.addField(Call::AlternativeCallsField("allele1AlternativeCalls", &d.alleleAlternativeCalls_[0]),
                    DelimitedFile::FPT_OPTIONAL);
        df.addField(Call::AlternativeCallsField("allele2AlternativeCalls", &d.alleleAlternativeCalls_[1]),
                    DelimitedFile::FPT_OPTIONAL);
        ...
        StringSet callColumns("locus,ploidy,chromosome,begin,end,zygosity,"
                              ...
                              "allele1VarFilter,allele2VarFilter,"
                              "allele1Freq,allele2Freq,"
                              "allele1AlternativeCalls,allele2AlternativeCalls,"
