...
protected:
    ...
    class MidpointField : public util::DelimitedFieldParser
    {
    public:
        ...
        void parse(const char* first, const char* last)
            {
                *begin_ = util::parseValue<Value>(first, last);
                *end_ = *begin_;
            }
    ...
    //! Utility function that the implementation of the derived class
    //! can call from bindColumns to add the parsers for the columns that
    //! correspond to range data, special cased for 
    void bindRangeMidpointColumns(util::DelimitedFile& df,
                          reference::Range& range,
                          const std::string& chrColName = "chromosome",
                          const std::string& midpointColName = "position")
    {
        df.addField(ChromosomeIdField(chrColName, &range.chromosome_, crr_));
        df.addField(MidpointField<uint32_t>(midpointColName, &range.begin_, &range.end_));
    }
